...
impl CourseOfActionBuilder {
    ...
    pub fn build(self) -> Result<CourseOfAction, super::BuilderError> {
        let name = self.name.ok_or(super::BuilderError::MissingField("name"))?;
        let common = CommonProperties::new("course-of-action", self.created_by_ref);
        Ok(CourseOfAction {
            common,
            name,
...
impl From<CourseOfAction> for crate::StixObjectEnum {
    fn from(c: CourseOfAction) -> Self {
        crate::StixObjectEnum::CourseOfAction(c)
    }
}
